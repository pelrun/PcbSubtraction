...
void process_string(char instruction[], int size)
{
	...
	if (instruction[0] == '/')	
		return;
	//the # character means wer are setting register values 
	else if(instruction[0] == '#'){
		int reg1, reg2;
		float regV;
		char *end1, *end2;
		reg1 = (int)strtol(&instruction[1], &end1, 10);
		if(end1[1] == '#'){
			reg2 = (int)strtol(&end1[2], &end2, 10);
			registers[reg1] = registers[reg2];
		}
		else{
  			regV = (float)strtod(&end1[1], &end2);
  			registers[reg1] = regV;
		}
		return;
	}
...
// No room for this in the Arduino
			...
			case 38: //for probing, currently no difference between G01 and G38, but dda_move was updated
				//set our target.
				set_target(fp.x, fp.y, fp.z);
			...
			case 39: //for exporting register values
				Serial.print("R:");
