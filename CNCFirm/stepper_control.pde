...
void dda_move(long micro_delay)
{
	...
	z_probe_tripped = false;
	//do our DDA line!
	do
	{
		x_can_step = canX_step(X_MIN_PIN, X_MAX_PIN, current_steps.x, target_steps.x, x_direction);
		y_can_step = canY_step(Y_MIN_PIN, Y_MAX_PIN, current_steps.y, target_steps.y, y_direction);
		z_can_step = canZ_step(Z_MIN_PIN, Z_MAX_PIN, current_steps.z, target_steps.z, z_direction);
	...
	if(!z_probe_tripped){
		...
		current_units.z = to_units(z_units, current_steps.z);
		z_probe_tripped = false;
	}
	registers[0] = current_units.z;
	calculate_deltas();
        disable_steppers();
}

bool canX_step(byte min_pin, byte max_pin, long current, long target, byte dir)
{
...
bool canY_step(byte min_pin, byte max_pin, long current, long target, byte dir)
{
	//stop us if we're on target
	if (target == current)
		return false;
...
bool canZ_step(byte min_pin, byte max_pin, long current, long target, byte dir)
{
	//stop us if we're on target
	if (target == current)
		return false;
	...
	//stop us if we're at max and still going
        ...
        //stop us if we hit probe 
	else if (!read_switch(min_pin) && !dir){
		z_probe_tripped = true;
		return false;
	}
...
void do_half_step(byte step_pin)
{
	digitalWrite(step_pin, HIGH);
...
float to_units(float steps_per_unit, long steps)
{
